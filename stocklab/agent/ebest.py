...
class EBest:
    ...
    def __init__(self, mode=None):
        ...
        self.cert_passwd = config[run_mode]["cert_password"]
    ...
    def _execute_query(self, res, in_block_name, out_block_name, *out_fields, **set_fields):
        ...
        while len(self.query_cnt) >= EBest.QUERY_LIMIT_10MIN:
            ...
            self.query_cnt = list(
                filter(
                    lambda x: (datetime.today() - x).total_seconds() < EBest.LIMIT_SECONDS,
                    self.query_cnt,
                )
            )
        ...
        print("XAQuery.RES_PATH", XAQuery.RES_PATH)
        print("res", res)
    ...
    def get_stock_price_by_code(self, code=None, cnt="1"):
        ...
        out_params = ['date', 'open', 'high', 'low', 'close', 'sign',
                    'change', 'diff', 'volumn', 'diff_vol', 'chdegree',
                    'sojinrate', 'changerate', 'fpvolumn', 'covolumn',
                    'value', 'ppvolume', 'o_sign', 'o_change', 'o_deff',
    ...
    def get_credit_trend_by_code(self, code=None, date=None):
        ...
        out_params = {"mmdate", "close", "sign", "jchange", "diff", "nvolumn",
                    "svolume", "jvolumn", "price", "change", "gyrate", "jkrate",
                    "shcode"}
